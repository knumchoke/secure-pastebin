...
- `go.mod`: `module github.com/knumchoke/secure-pastebin`, `go 1.26` (raised from 1.25 during WS1: current `golang.org/x/*` releases require 1.26).
...
mkdir -p bin && GOBIN=$PWD/bin go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@latest   # project-local v2; `make tools` does this too
```
...
linters:
  ...
  exclusions:
    rules:
      - path: _test\.go
        linters: [noctx] # httptest.NewRequest in handler tests is fine
```
...
paths:
  ...
  /pastes/{id}/unlock:
    post:
      ...
      responses:
        "200":
          description: "unlocked. With Accept: text/plain the raw canonical bytes are returned as an attachment."
          content:
...
Run: `rtk go get github.com/getkin/kin-openapi@latest && rtk go test ./docs/api/ -v` — Expected: PASS (the document loads and validates)
...
## Execution notes (deviations made while executing — the repo is the truth)

- Go floor is **1.26** (current `golang.org/x/*` releases require it). `go mod tidy` writes `go 1.26.0`.
- `golangci-lint` v2 is installed project-locally into `./bin/` (`make tools`); the Makefile prefers `./bin/golangci-lint`. Config excludes `errcheck` for `fmt.Fprint*` and `noctx` for `_test.go`.
- Contract test validates `openapi.yaml` with `kin-openapi` (pulled forward from WS5). Flow-style YAML descriptions containing `,` or `:` must be quoted.
- Dockerfile: no `apk add` (blocked by TLS interception and unnecessary); optional `deploy/ca/*.crt` is trusted in both build stages; `.dockerignore` keeps `.git`, `deploy/secrets`, `deploy/.env`, `node_modules` out of the context; compose healthcheck uses the `["CMD", ...]` form.
- `ui/package.json` test script uses `--passWithNoTests` so `make check` is green before WS6.
